	...
	while(attempt < 3)
	{
		...
		try
		{
			if(driver.findElement(oElement).isDisplayed())
			{
		        ...
		        action.moveToElement(element).build().perform();
		        hoverthemouseintoelement = true;
		        System.out.println("Perform the mouse hover operation on "+driver.findElement(oElement).getAttribute("text"));
		        break;
			}
		}
		catch(Exception e)
		{
			System.out.println("Exception while locatiing element");
			e.printStackTrace();
			System.out.println("hoverTheMouseIntoElement - Exception Occured");
			try
			{ 
				driver.navigate().refresh(); 
			}
